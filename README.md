...
Unfortunately, it seems there is no other parser/walker with a similarly easy learning curve and which support types other than single bit booleans/`std_logic`:

- GHDL produces waveform dumps in multiple formats. Hence, its codebase contains the required logic to achieve the desired purpose. However, it is not intuitive how to get the name and value of the signals that are being dumped: [ghdl/ghdl · src/grt/grt-waves.adb#L1781-L1783](https://github.com/ghdl/ghdl/blob/master/src/grt/grt-waves.adb#L1781-L1783).
- By the same token, [GtkWave](http://gtkwave.sourceforge.net/) constains C sources that allow to read the waveform formats generated by GHDL. However, those are not meant to be used as a toolkit (see [gtkwave/gtkwave#9](https://github.com/gtkwave/gtkwave/issues/9)).
